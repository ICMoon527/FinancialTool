...
class KD(BaseIndicator):
    """
    KD（随机指标），中文称为KD指标。

    KD指标是由George Lane提出的，是一种震荡指标。
    KD指标通过计算收盘价在一段时间内的位置，来判断价格的超买超卖状态。
    ...
    def calculate(self, data):
        ...
        df['K'] = df['RSV'].ewm(span=self.k_period, adjust=False).mean()
        df['D'] = df['K'].ewm(span=self.d_period, adjust=False).mean()
        df['J'] = 3 * df['K'] - 2 * df['D']
