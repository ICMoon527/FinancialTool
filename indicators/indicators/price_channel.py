...
class PriceChannel(BaseIndicator):
    """
    Price Channel（价格通道指标），中文称为价格通道指标。

    价格通道指标是一种基于最高价和最低价的指标，通过计算N日的最高价和最低价，
    来判断价格的突破和趋势变化。

    计算公式：
    1. 计算上轨：Upper = HHV(High, N)
    2. 计算下轨：Lower = LLV(Low, N)
    3. 计算中轨：Middle = (Upper + Lower) / 2
    ...
    - upper_rising: 上轨上升信号
    - lower_falling: 下轨下降信号
    - upper_falling: 上轨下降信号
    - lower_rising: 下轨上升信号
    - channel_width: 通道宽度
    ...
    最佳实践建议：
    1. 趋势确认：价格突破上轨且上轨向上更可靠
    2. 组合使用：与MACD等趋势指标配合使用
    3. 多周期使用：同时使用日线和周线的价格通道
    4. 量价配合：结合成交量变化验证信号
    5. 趋势过滤：先判断大趋势，顺势操作
    6. 止损设置：设置严格止损，控制风险
    7. 灵活调整：根据市场波动性调整参数
    ...
    def calculate(self, data):
        ...
        df['upper_rising'] = df['Upper'] > df['Upper'].shift(1)
        df['lower_falling'] = df['Lower'] < df['Lower'].shift(1)
        df['upper_falling'] = df['Upper'] < df['Upper'].shift(1)
        df['lower_rising'] = df['Lower'] > df['Lower'].shift(1)
        ...
        return df[['Upper', 'Lower', 'Middle', 'close_above_upper', 'close_below_lower', 
                   'close_above_middle', 'close_below_middle', 'upper_rising', 'lower_falling',
                   'upper_falling', 'lower_rising', 'channel_width', 
                   'channel_expanding', 'channel_shrinking']]
