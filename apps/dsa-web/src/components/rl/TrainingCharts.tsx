...
const LossChart: React.FC<{ data: Partial<RLMetricsHistory> }> = ({ data }) => {
  ...
  const option: EChartsOption = {
    ...
    yAxis: [
      { type: 'value', name: 'Loss', axisLabel: { color: '#a0a0b0' }, splitLine: { lineStyle: { type: 'dashed', opacity: 0.3 } } },
      { type: 'value', name: 'TD', axisLabel: { color: '#a0a0b0' }, splitLine: { show: false } },
    ],
    series: [
...
const ValChart: React.FC<{ data: Partial<RLMetricsHistory> }> = ({ data }) => {
  ...
  const option: EChartsOption = {
    ...
    yAxis: [
      { type: 'value', name: 'Sharpe', axisLabel: { color: '#a0a0b0' }, splitLine: { lineStyle: { type: 'dashed', opacity: 0.3 } } },
      { type: 'value', name: '收益率', axisLabel: { color: '#a0a0b0', formatter: (v: number) => `${(v * 100).toFixed(1)}%` }, splitLine: { show: false } },
    ],
    series: [
