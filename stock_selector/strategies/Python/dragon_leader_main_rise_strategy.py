# -*- coding: utf-8 -*-
"""
Dragon Leader Main Rise Strategy - Python implementation.
"""
...
from stock_selector.strategies.Python.dragon_leader_main_rise import DragonLeaderMainRise

logger = logging.getLogger(__name__)


@register_strategy
class DragonLeaderMainRiseStrategy(StockSelectorStrategy):
    """
    Dragon Leader Main Rise Strategy - Python implementation.
    
    This strategy screens stocks based on the Dragon Leader Main Rise indicator,
    which identifies the main rise wave signal of leading stocks.
    """

    def __init__(self):
        metadata = StrategyMetadata(
            id="dragon_leader_main_rise",
            name="dragon_leader_main_rise",
            display_name="龙头主升策略",
            description="识别龙头股主升浪信号的策略，整合主升追踪、量能异动、换手热度、主力流向四个指标",
        ...
        self.indicator = DragonLeaderMainRise()

    def select(self, stock_code: str, stock_name: Optional[str] = None) -> StrategyMatch:
        """
        Execute the Dragon Leader Main Rise strategy for a single stock.
                ...
                if daily_data is not None and isinstance(daily_data, pd.DataFrame) and not daily_data.empty:
                    ...
                    try:
                        ...
                        if len(indicator_result) > 0:
                            ...
                            if latest.get('dragon_rise_strong', False):
                                conditions_met.append("龙头主升强信号")
                                total_score += 40
                                match_details["conditions"]["strong_signal"] = {"passed": True}
                            elif latest.get('dragon_rise_moderate', False):
                                conditions_met.append("龙头主升中等信号")
                                total_score += 25
                                match_details["conditions"]["moderate_signal"] = {"passed": True}
                            else:
                                conditions_failed.append("未检测到龙头主升信号")
                                match_details["conditions"]["signal"] = {"passed": False}

                            if latest.get('dragon_rise_signal', False):
                                conditions_met.append("主升信号触发")
                                total_score += 30
                                match_details["conditions"]["rise_signal"] = {"passed": True}
