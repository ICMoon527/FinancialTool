...
class Config:
    ...
    enable_multithreading: bool = True  # 是否启用多线程模式
    debug: bool = False
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # OPENAI_API_KEYS > AIHUBMIX_KEYS > AIHUBMIX_KEY > OPENAI_API_KEY
        _openai_keys_raw = os.getenv('OPENAI_API_KEYS', '')
        openai_api_keys = [k.strip() for k in _openai_keys_raw.split(',') if k.strip()]
        if not openai_api_keys:
            _aihubmix_keys_raw = os.getenv('AIHUBMIX_KEYS', '')
            _aihubmix_keys = [k.strip() for k in _aihubmix_keys_raw.split(',') if k.strip()]
            if _aihubmix_keys:
                openai_api_keys = _aihubmix_keys
            else:
