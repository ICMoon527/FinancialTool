    ...
    def generate_dashboard_report(
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                # ========== 技术指标分析 ==========
                # 检查是否有主力成本数据（直接从分析上下文中获取）
                macdfs_data = None
                if hasattr(result, 'dashboard') and result.dashboard and 'macdfs' in result.dashboard:
                    macdfs_data = result.dashboard['macdfs']
                # 尝试从原始上下文中获取（如果存在）
                elif hasattr(result, '_context') and result._context and 'macdfs' in result._context:
                    macdfs_data = result._context['macdfs']
                ...
                if macdfs_data:
                    report_lines.extend([
                        ...
                        "#### 主力成本分析（MACDFS指标）",
                        "",
                        "| 指标 | 数值 | 计算依据 | 市场含义 | 操作建议 |",
                        "|------|------|----------|----------|----------|",
                    ])
                    
                    # 核心综合成本
                    core_cost = macdfs_data.get('core_cost', 'N/A')
                    report_lines.append(f"| **核心综合成本** | **{core_cost}** | 多周期量价加权成本线 | 主力平均持仓成本，价格突破此线视为强势 | 价格在成本线附近可考虑低吸 |")
                    
                    # 第一压力位
                    resistance_1 = macdfs_data.get('resistance_1', 'N/A')
                    report_lines.append(f"| **第一压力位** | **{resistance_1}** | 核心成本×1.03 | 短期关键阻力位，突破后可能加速上涨 | 接近此位可考虑减仓或设置止盈 |")
                    
                    # 第二压力位
                    resistance_2 = macdfs_data.get('resistance_2', 'N/A')
                    report_lines.append(f"| **第二压力位** | **{resistance_2}** | 核心成本×1.13 | 中期重要阻力位，突破后空间打开 | 到达此位建议大幅减仓或清仓 |")
                    
                    # 长期成本轨道
                    long_term_orbit_1 = macdfs_data.get('long_term_orbit_1', 'N/A')
                    report_lines.append(f"| **长期成本轨道1** | {long_term_orbit_1} | 长期成本支撑线 | 长期趋势支撑位，跌破可能转熊 | 此线附近是重要买入机会 |")
                    
                    long_term_orbit_2 = macdfs_data.get('long_term_orbit_2', 'N/A')
                    report_lines.append(f"| **长期成本轨道2** | {long_term_orbit_2} | 动态成本支撑线 | 中期趋势支撑位，跌破需警惕 | 回调至此线可考虑分批建仓 |")
                    
                    # 强势突破信号
                    strong_breakout = macdfs_data.get('strong_breakout_signal', 0)
                    breakout_status = "✅ 触发" if strong_breakout == 1 else "❌ 未触发"
                    breakout_advice = "可考虑追涨" if strong_breakout == 1 else "观望为主"
                    report_lines.append(f"| **强势突破信号** | {breakout_status} | 涨>3%且收盘价=最高价 | 短期强势信号，可能开启主升浪 | {breakout_advice} |")
                    
                    # 波动率因子
                    volatility_factor = macdfs_data.get('volatility_factor', 'N/A')
                    report_lines.append(f"| **波动率因子** | {volatility_factor} | 价格波动强度 | 数值越大，波动越剧烈 | 高波动时需控制仓位 |")
        ...
        report_lines.extend([
            ...
            "",
            "**识别与应用**:",
            "- **支撑位识别**: 前期低点、均线、趋势线、整数关口等都是潜在支撑位",
            "- **买入时机**: 价格接近支撑位时，观察量能变化，缩量企稳可考虑买入",
            "- **信号确认**: 支撑位附近出现反转K线形态（如锤子线、启明星）是买入信号",
            "- **风险管理**: 跌破支撑位后回抽确认失败，应及时止损",
            "- **仓位控制**: 支撑位买入时应分批建仓，避免一次性满仓",
            "",
            "### 压力位交易策略",
            "",
            "**识别与应用**:",
            "- **压力位识别**: 前期高点、均线、趋势线、整数关口等都是潜在压力位",
            "- **卖出时机**: 价格接近压力位时，如量能不足应考虑减仓",
            "- **突破确认**: 放量突破压力位并回踩确认，可加仓跟进",
            "- **风险提示**: 多次冲击压力位失败，应及时获利了结",
            "- **止盈设置**: 压力位附近可设置阶梯式止盈，保护已有利润",
            "",
            "### 不同市场情境策略",
            "",
            "**牛市策略**:",
            ...
            "- 支撑位买入，压力位卖出，高抛低吸操作",
            "- 严格执行交易计划，避免情绪化决策",
            "- 震荡区间突破时及时跟随，突破方向决定策略",
            "",
            "### 技术指标配合",
            "",
            "- **成交量**: 突破时量能放大是有效突破的重要信号",
            "- **MACD**: 金叉/死叉与支撑位/压力位配合使用",
            "- **RSI**: 超买/超卖状态在支撑位/压力位附近更有参考价值",
            "- **KDJ**: 低位金叉+支撑位，高位死叉+压力位是经典组合",
            "",
            "### 风险控制原则",
            "",
            "- 严格设置止损，单次交易风险控制在总资金的2-3%",
            "- 支撑位买入后跌破支撑位必须止损",
            "- 压力位卖出后突破压力位可重新买入",
            "- 保持交易纪律，避免频繁交易和追涨杀跌",
            "- 结合基本面分析，避免纯技术面交易的局限性",
            "",
        ])
