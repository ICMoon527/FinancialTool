...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def _call_litellm(self, prompt: str, generation_config: dict) -> str:
        ...
        # 只使用配置的主模型，不使用fallback模型
        # 这样可以确保只使用阿里云的模型
        models_to_try = [config.litellm_model]
    ...
    def analyze(
        ...
        try:
            ...
            # 解析响应
            result = self._parse_response(response_text, code, name, context)
...
请输出完整的 JSON 格式决策仪表盘。"""
    ...
    def _parse_response(
        ...
        try:
            ...
            if data is not None:
                name = extract_stock_name(data, name, code)
                one_sentence = extract_one_sentence_decision(data)
                no_position_advice, has_position_advice = extract_position_advice(data, one_sentence)
                sniper_points = extract_sniper_points(data)
                ...
                logger.info(f"解析完成: {name}({code}), 评分={sentiment_score}, 建议={operation_advice}")
                return result
            else:
        ...
        return AnalysisResult(
            ...
            operation_advice='观望',
            confidence_level='低',
            analysis_summary='解析失败，建议手动分析',
            risk_warning='分析结果解析失败，请稍后重试',
            success=False,
            error_message='解析失败'
        )
