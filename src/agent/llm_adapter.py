                    ...
                    for choice in chunk.choices:
                        if hasattr(choice, 'delta'):
                            ...
                            if hasattr(choice.delta, 'reasoning_content') and choice.delta.reasoning_content:
                                collected_reasoning += choice.delta.reasoning_content
            # 如果收集到内容，创建一个模拟的响应对象
            if collected_content:
                ...
                class MockResponse:
                    def __init__(self, content, reasoning_content=None):
                        ...
                        # 模拟message对象
                        class MockMessage:
                            def __init__(self, content, role, reasoning_content=None):
                                self.content = content
                                self.role = role
                                self.reasoning_content = reasoning_content
                        ...
                        message = MockMessage(content, "assistant", reasoning_content)
                        choices = MockChoices(0, message, delta, "stop")
                        self.choices = [choices]
                        self.usage = None
                response = MockResponse(collected_content, collected_reasoning)
