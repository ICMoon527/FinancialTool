...
from src.core.json_parser import (
    ...
    is_aliyun_simple_format,
    parse_aliyun_simple_format,
)
...
class StockAnalysisPipeline:
    ...
    def fetch_and_save_stock_data(
        ...
        Args:
            ...
            force_history: 是否强制获取历史数据（即使有今日数据）
        ...
        try:
            ...
            if not force_refresh and not force_history and self.db.has_today_data(code, today):
                ...
                if len(history_bars) >= 100:
                    logger.info(f"{stock_name}({code}) 今日数据已存在且历史数据充足，跳过获取（断点续传）")
                    return True, None
                else:
                    logger.info(f"{stock_name}({code}) 今日数据已存在但历史数据不足，需要获取历史数据")
            else:
        ...
        try:
            ...
            if result:
                ...
                result = AnalysisResult(
                    ...
                    operation_advice='持有',
                    confidence_level='低',
                    analysis_summary='分析过程出错: AI 分析失败',
    ...
    def _agent_result_to_analysis_result(
        ...
        if agent_result.success and agent_result.dashboard:
            ...
            # 检查是否是阿里云简单格式
            if is_aliyun_simple_format(dash):
                logger.info(f"[{code}] 检测到阿里云简单格式，使用专用解析器")
                parsed_data = parse_aliyun_simple_format(dash)
                
                ai_stock_name = parsed_data.get("stock_name", "")
                if ai_stock_name and self._is_placeholder_stock_name(stock_name, code):
                    result.name = ai_stock_name
                
                result.sentiment_score = parsed_data.get("sentiment_score", 50)
                result.trend_prediction = parsed_data.get("trend_prediction", "未知")
                result.operation_advice = parsed_data.get("operation_advice", "观望")
                result.decision_type = parsed_data.get("decision_type", "hold")
                result.analysis_summary = parsed_data.get("analysis_summary", "")
                result.dashboard = parsed_data.get("dashboard")
                result.key_points = parsed_data.get("key_points", "")
                result.risk_warning = parsed_data.get("risk_warning", "")
                result.buy_reason = parsed_data.get("buy_reason", "")
                result.trend_analysis = parsed_data.get("trend_analysis", "")
                result.short_term_outlook = parsed_data.get("short_term_outlook", "")
                result.medium_term_outlook = parsed_data.get("medium_term_outlook", "")
                result.technical_analysis = parsed_data.get("technical_analysis", "")
                result.ma_analysis = parsed_data.get("ma_analysis", "")
                result.volume_analysis = parsed_data.get("volume_analysis", "")
                result.pattern_analysis = parsed_data.get("pattern_analysis", "")
                result.fundamental_analysis = parsed_data.get("fundamental_analysis", "")
                result.sector_position = parsed_data.get("sector_position", "")
                result.company_highlights = parsed_data.get("company_highlights", "")
                result.news_summary = parsed_data.get("news_summary", "")
                result.market_sentiment = parsed_data.get("market_sentiment", "")
                result.hot_topics = parsed_data.get("hot_topics", "")
                result.success = True
            elif 'sentiment_score' in dash and 'dashboard' in dash:
                ...
                result.key_points = dash.get("key_points", "")
                result.risk_warning = dash.get("risk_warning", "")
            else:
                ai_stock_name = extract_stock_name(dash, stock_name, code)
                if ai_stock_name and self._is_placeholder_stock_name(stock_name, code):
                    result.name = ai_stock_name
                
                one_sentence = extract_one_sentence_decision(dash)
                no_position_advice, has_position_advice = extract_position_advice(dash, one_sentence)
                sniper_points = extract_sniper_points(dash)
                checklist = extract_checklist(dash)
                
                signal_type, sentiment_score, operation_advice, decision_type, trend_prediction = determine_signal_and_score(one_sentence)
                ...
                risk_warning = extract_risk_warning(checklist)
                ...
                result.analysis_summary = one_sentence
    ...
    def run(
        ...
        # 检查是否启用多线程
        if getattr(self.config, 'enable_multithreading', True):
            # 使用线程池并发处理
            ...
            # 单线程执行
            logger.info(f"多线程模式已禁用，使用单线程执行 {len(stock_codes)} 只股票的分析")
